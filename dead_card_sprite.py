...
class Dead_Card_Sprite(pygame.sprite.Sprite): #Subclass of pygame.Sprite
    screen = None
    coords = [0, 0]
    screen_info = None
    screen_x = 1080
    ...
    def __init__(self, card_sprite):
        ...
        self.uid = card_sprite.uid
        self.is_rotated = card_sprite.is_rotated
        #rest of class stuff goes here
        self.screen = card_sprite.screen
        self.set_coords(card_sprite.slot)
        self.image = pygame.image.load("CartArt/red_x.png").convert() #adds image from file
        self.image = pygame.transform.scale(self.image, (self.card_width, self.card_length)) #Scales image to constant size
        if self.slot[0] == 2 or self.slot[0] == 1:
